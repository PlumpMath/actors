...
public final class UnstructuredClientCoroutine implements Coroutine {
    ...
    public void run(Continuation cnt) throws Exception {
        ...
        ctx.addOutgoingMessage(ctx.getSelf(), logAddress, info("Starting client with seed {} and bootstrap {}", seed, bootstrapLinks));
        ctx.addOutgoingMessage(ctx.getSelf(), graphAddress, new AddNode(addressTransformer.toLinkId(ctx.getSelf())));

        Address routerAddress = ctx.getSelf().append(ROUTER_RELATIVE_ADDRESS);
        SubcoroutineRouter outgoingLinkRouter = new SubcoroutineRouter(routerAddress, ctx);
        Controller controller = outgoingLinkRouter.getController();
        
        // start subcoroutine to deal with incoming requests
        ctx.addOutgoingMessage(ctx.getSelf(), logAddress, debug("Adding handler"));
        Address incomingMessageHandlerAddress = ctx.getSelf().append(ROUTER_HANDLER_RELATIVE_ADDRESS);
        controller.add(
                incomingMessageHandlerAddress,
        ...
        ctx.addOutgoingMessage(ctx.getSelf(), logAddress, debug("Adding querier"));
        Address outgoingQueryAddress = ctx.getSelf().append(ROUTER_QUERIER_RELATIVE_ADDRESS);
        controller.add(
                outgoingQueryAddress,
        ...
        for (int i = 0; i < state.getMaxOutgoingLinks(); i++) {
            ctx.addOutgoingMessage(ctx.getSelf(), logAddress, debug("Adding outgoing link maintainer {}", i));
            String elementName = String.format(OUT_ELEMENT_NAME_FORMAT, i);
            Address outgoingLinkAddress = ctx.getSelf().append(ROUTER_RELATIVE_ADDRESS).append(elementName);
            controller.add(
                    outgoingLinkAddress,
